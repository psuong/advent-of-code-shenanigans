...
fn is_data_valid(index : &i32, word : &str) -> bool {
    ...
    // println!("Value at idx {} for {}", FIELD.get(idx).unwrap(), value);
    match index {
        0 => {
        ...
        3 => {
           match value.replace(INCH, "").replace(CENTIMETER, "").parse::<i32>() {
               Ok(n) => return (n >= 150 && n <= 193) || (n >= 59 && n <= 76),
               Err(_err) => return false
           }
        },
        4 => {
    ...
    if let Ok(lines) = read_lines("resources/day4.txt") {
        ...
        for (_line_number, line) in lines.enumerate() {
            ...
            if line_length == 0 {
                ...
                if mask >= REQUIRED_MASK && min_mask == REQUIRED_MASK {
                    valid_passports += 1;
                }
                mask = 0;
            };
